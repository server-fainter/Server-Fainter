...
void broadcast_to_clients(unsigned char *message, size_t len) {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (clients[i].active && clients[i].handshake_done) {
            ...
            uint8_t header[14]; // 2 bytes 기본 헤더 + 8 bytes 페이로드 길이 + 4 bytes (필요시)
            ...
            if (len <= 125) {
                ...
                header[1] = 127;
                // 네트워크 바이트 오더 (big-endian)로 8바이트 페이로드 길이 설정
                for (int j = 0; j < 8; j++) {
                    header[2 + j] = (len >> (56 - 8*j)) & 0xFF;
                }
                header_size = 10;
            }
...
ssize_t process_websocket_frame(client_t *cli) {
    ...
    size_t masking_key_offset = header_len;
    if (masked) {
        header_len += 4;
    }
